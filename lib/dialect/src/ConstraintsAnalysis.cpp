...
static bool isPotentialRelationalOp(mlir::Operation* op){
	return 
		mlir::isa<mlir::rlc::GreaterOp>(op) or	
...
// Method that peeks the lattice for the ranges and updates the "result" value accordingly
// based on the arithmetic operation
	...
	bool branch)
{
	...
	if (not currentLattice->isValueNotTopOrBottom(result, branch)) return false;
	...
	if(auto other_casted=mlir::dyn_cast<mlir::rlc::Constant>(other.getDefiningOp())){
		...
		if(auto other_const=other_casted->getAttr("value").dyn_cast<mlir::IntegerAttr>()){
	...
	// Else check if the value carried by the operation is 
		...
		if(not currentLattice->isValueNotBottom(other,branch)) return false;
	...
	if(new_range.getLower().sle(new_range.getUpper())){
		...
		.Case<mlir::rlc::AddOp>([&](mlir::rlc::AddOp addop){
			new_range=result_range.subWithNoWrap(new_range,0,llvm::ConstantRange::PreferredRangeType::Signed);
		});
...
// Method that given an arithmetic operation finds the operand to update (lhs or rhs) 
...
static bool findRelevantOperandAndUpdate(
	...
	else if(op2_defop == nullptr or not mlir::isa<mlir::rlc::Constant>(op2_defop)){
		update=op2;
		other=op1;
	}
	else return false;

	bool ret1=calculateNewRangeAndUpdate(op, currentLattice, analysis, result, update, other, true);
	bool ret2=calculateNewRangeAndUpdate(op, currentLattice, analysis, result, update, other, false);
...
static const std::tuple<bool,bool,bool,bool> GetBranchesCondBranch(
	...
	// Understand if there is a value or a constant (useful for corner cases after)
...
// Method to retrieve the result range from a relational operation and insert it in the lattice
static bool getRangeOfRelationalOperation(
	...
	if(auto rhs_integer=other_casted->getAttr("value").dyn_cast<mlir::IntegerAttr>())
	...
	// Insert the range where is needed
	return currentLattice->insertOrUpdate({to_update,branch_to_update},min,max);
}
...
static bool visitYield(
	...
	if(rhs_op!=nullptr){ // Corner case because if rhs is a blockargument this is null
		if(auto rhs_casted=mlir::dyn_cast<mlir::rlc::Constant>(rhs_op)){

			to_return=getRangeOfRelationalOperation(currentLattice, conditional_op, true, lhs, rhs_casted, true);
			to_return=getRangeOfRelationalOperation(currentLattice, conditional_op, false, lhs, rhs_casted, false) or to_return;
	...
	// else do the same but with inverted branches
		...
		if(auto lhs_casted=mlir::dyn_cast<mlir::rlc::Constant>(lhs_op)){
			...
			to_return=getRangeOfRelationalOperation(currentLattice, conditional_op, false, rhs, lhs_casted, true);	
			to_return=getRangeOfRelationalOperation(currentLattice, conditional_op, true, rhs, lhs_casted, false) or to_return;
...
static bool setReturnValueInLattice(mlir::Operation* branchOp, 
	mlir::rlc::ConstraintsLattice* currentLattice){
	...
	auto bool_branch= mlir::dyn_cast<mlir::BoolAttr>(value.getDefiningOp()->getAttr("value"));
	if (not bool_branch)
		return false;

	return currentLattice->insertOrJoin({value, bool_branch.getValue()},
		currentLattice->getTOP());
}
...
bool mlir::rlc::ConstraintsAnalyzable::constraintsAnalyze(
	mlir::rlc::ConstraintsLattice* currentLattice, mlir::rlc::ConstraintsAnalysis* analysis)
{
	...
	mlir::rlc::ConstraintsLattice* currentLattice, mlir::rlc::ConstraintsAnalysis* analysis)
{
	...
	return findRelevantOperandAndUpdate(this->getOperation(),currentLattice, analysis,result);
...
bool mlir::rlc::Yield::constraintsAnalyze(
	...
	mlir::rlc::ConstraintsAnalysis* analysis)
{
	// Just return the lattice method, as rlc.builtin_assign will need the same call too
	...
	mlir::rlc::ConstraintsAnalysis* analysis)
{
	...
	if(auto uninit_construct=mlir::dyn_cast<mlir::rlc::UninitializedConstruct>(casted.getCond().getDefiningOp())){
	...
	if(auto rhs_casted=mlir::dyn_cast<mlir::rlc::Constant>(rhs_op)){
		...
		std::tie(btyt, btyf, bfyt, bfyf)
			=GetBranchesCondBranch(currentLattice, this->getOperation(), analysis, lhs);
		...
		if(not btyt==bfyt){
			...
			if(btyt)
				to_return=getRangeOfRelationalOperation(currentLattice, conditional_op, true, lhs, rhs_casted, true) or to_return;	

			if(bfyt)
				to_return=getRangeOfRelationalOperation(currentLattice, conditional_op, false, lhs, rhs_casted, true) or to_return;
		...
		if(not btyf==bfyf){
			...
			if(btyf)
				to_return=getRangeOfRelationalOperation(currentLattice, conditional_op, true, lhs, rhs_casted, false) or to_return;
	...
	// If the lhs is a constant then do the same but with inverted branches
		...
		std::tie(btyt, btyf, bfyt, bfyf)
			=GetBranchesCondBranch(currentLattice, this->getOperation(), analysis, rhs);
		...
		if(not btyt==bfyt){
			...
			if(btyt)
				to_return=getRangeOfRelationalOperation(currentLattice, conditional_op, false, rhs, lhs_casted, true) or to_return;	
			
			if(bfyt)
				to_return=getRangeOfRelationalOperation(currentLattice, conditional_op, false, rhs, lhs_casted, false) or to_return;	
		...
		if(not btyf==bfyf){
			...
			if(btyf)
				to_return=getRangeOfRelationalOperation(currentLattice, conditional_op, true, rhs, lhs_casted, true) or to_return;	
...
bool mlir::rlc::BuiltinAssignOp::constraintsAnalyze(
	...
	mlir::rlc::ConstraintsAnalysis* analysis)
{
	auto casted=mlir::dyn_cast<mlir::rlc::BuiltinAssignOp>(this->getOperation());
	...
	// When i have statements such as 'a=a+1' I can check them with builtin assigns
	if(auto rhs_casted=mlir::dyn_cast<mlir::rlc::AddOp>(casted->getOperand(1).getDefiningOp())){
		return findRelevantOperandAndUpdate(casted->getOperand(1).getDefiningOp(), currentLattice, analysis, casted->getOperand(0));
	}
...
namespace mlir::rlc
{
	...
	const llvm::DenseMap<mlir::rlc::CA_Value,ConstraintsLattice::IntegerRange>& ConstraintsLattice::getUnderlyingLattice() const{
		return this->ranges;
	}

	// This is be the opposite of a join, but for the analysis we can treat it as a join
	...
	void ConstraintsLattice::print(raw_ostream& os) const{
		...
		if(not this->ranges.empty()){
			for(auto range: this->ranges){
				os << "SSA: "<< range.first.value <<" Branch: "<< range.first.return_type << 
	...
	// Useful method
	bool ConstraintsLattice::isValueNotTopOrBottom(const mlir::Value& value, bool branch) const{
	...
	// Useful method
	bool ConstraintsLattice::isValueNotBottom(const mlir::Value& value, bool branch) const{
		auto found=this->ranges.find({value,branch});
	...
	// Useful method
	const ConstraintsLattice::IntegerRange& ConstraintsLattice::getRange(const mlir::Value& value, bool branch) const{
		return this->ranges.find({value,branch})->second;
	}
	
	// Useful method for checking which branches(T/F) are saved in the current lattice (rlc.constant or even a value)
	...
	// Method for checking if the value in input is contained in both branches in the lattice
	...
	// Useful method
	bool ConstraintsLattice::rangesEmpty() const{
	...
	bool ConstraintsLattice::insertOrJoin(const mlir::rlc::CA_Value& key, const ConstraintsLattice::IntegerRange& other){
	...
	bool ConstraintsLattice::insertOrMeet(const mlir::rlc::CA_Value& key, const ConstraintsLattice::IntegerRange& other){
		auto found=this->ranges.find(key);
		if(found!=this->ranges.end()){
			auto copy=found->second;
			found->second= 
				ConstraintsLattice::meetRange(found->second, other);
			return copy!=found->second;
		}
		else{
			this->ranges.insert(std::make_pair(key, other));
	...
	bool ConstraintsLattice::insertOrUpdate(const mlir::rlc::CA_Value& key, const ConstraintsLattice::IntegerRange& other){
		auto found=this->ranges.find(key);
		if(found!=this->ranges.end()){
			auto copy=found->second;
			found->second=other;
	...
	bool ConstraintsLattice::insertOrUpdate(const mlir::rlc::CA_Value& keyToInsert, const std::optional<Integer>& min, const std::optional<Integer>& max){
	...
	void ConstraintsAnalysis::visitOperation(
		...
		if(auto casted=mlir::dyn_cast<mlir::rlc::ConstraintsAnalyzable>(op)){
			...
			if(mlir::isa<mlir::rlc::CondBranch>(casted) or mlir::isa<mlir::rlc::Yield>(casted))
	...
	// Useful method
	const llvm::DenseMap<mlir::rlc::CA_Value,mlir::rlc::ConstraintsLattice::IntegerRange>& ConstraintsAnalysis::getUnderlyingLattice(mlir::Operation* op){
	...
	class Constraints
	{
		...
		// Useful method
		const llvm::DenseMap<mlir::rlc::CA_Value,mlir::rlc::ConstraintsLattice::IntegerRange>& getOperationLattice(mlir::Operation* op){
			...
			: impl::ConstraintsAnalysisPassBase<ConstraintsAnalysisPass>
	{
		...
		void runOnOperation() override
		{
			...
			for (auto fun : getOperation().getOps<mlir::rlc::FlatFunctionOp>())
			{
				...
				// Moreover if the function has no arguments or are all not integer do nothing
				...
				const auto& block=fun.getRegion(0).front().getNextNode()->getNextNode();
