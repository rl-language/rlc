...
namespace llvm{
	template <>
	struct llvm::DenseMapInfo<mlir::rlc::CA_Value>{
		...
		static unsigned getHashValue(const mlir::rlc::CA_Value &Val) {
			return hash_combine(DenseMapInfo<mlir::Value>::getHashValue(Val.value), Val.return_type);
		}
		static bool isEqual(const mlir::rlc::CA_Value &lhs, const mlir::rlc::CA_Value &rhs) {
			return DenseMapInfo<mlir::Value>::isEqual(lhs.value, rhs.value) && lhs.return_type == rhs.return_type;
		}
	};
}
...
namespace mlir::rlc
{
	...
	class ConstraintsLattice: public mlir::dataflow::AbstractDenseLattice
	{
		...
		static const IntegerRange createRange(const Integer& min, const Integer& max){
		...
		// TODO: maybe inline
		static const IntegerRange joinRange(const IntegerRange& range1, const IntegerRange& range2){
		...
		static const IntegerRange meetRange(const IntegerRange& range1, const IntegerRange& range2){
			// TODO: understand this weird PreferredRangeType
			return range1.intersectWith(range2,IntegerRange::PreferredRangeType::Signed);
		}


		// Create a custom static method for updating a range
		// TODO: maybe inline
		static const IntegerRange updateRange(const IntegerRange& range1, const std::optional<Integer>& min, const std::optional<Integer>& max){
			return createRange(
		...
		bool insertOrJoin(const mlir::rlc::CA_Value& key, const IntegerRange& other);

		bool insertOrMeet(const mlir::rlc::CA_Value& key, const IntegerRange& other);

		bool insertOrUpdate(const mlir::rlc::CA_Value& key, const IntegerRange& other);

		bool insertOrUpdate(const mlir::rlc::CA_Value& keyToInsert, const std::optional<Integer>& min, const std::optional<Integer>& max);
						...
						ConstraintsLattice>
	{
		...
		const llvm::DenseMap<mlir::rlc::CA_Value,mlir::rlc::ConstraintsLattice::IntegerRange>& getUnderlyingLattice(mlir::Operation* op);
