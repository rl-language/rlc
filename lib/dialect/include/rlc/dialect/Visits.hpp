...
namespace rlc
{
	...
	inline llvm::SmallVector<mlir::Type, 4> postOrderTypes(mlir::ModuleOp module)
	{
		...
		for (auto decl : module.getOps<mlir::rlc::TypeAliasOp>())
		{
			if (emitted.contains(decl.getAliased()))
				continue;

			emitted.insert(decl.getAliased());

			decl.getAliased().walk(visit);

			subElements.push_back(decl.getAliased());
		}
