	...
	for (auto _ : state)
	{
		for (int i = 0; i < state.range(); i++)
			benchmark::DoNotOptimize(
					dictionary.insert(std::make_pair(KeyType(i), i)));
	}
	state.SetComplexityN(state.range(0));
}
...
static void BM_InsertRandom(benchmark::State& state)
{
	...
	for (auto _ : state)
	{
		for (int i = 0; i < state.range(); i++)
			benchmark::DoNotOptimize(
					dictionary.insert(std::make_pair(KeyType(unique_numbers[i]), i)));
	}
	state.SetComplexityN(state.range(0));
}
...
#define REGISTER_BENCHMARK_SUITE(DictType, KeyType, NameSuffix)                \
	...
	REGISTER_BENCHMARK(                                                          \
			BM_SubsciptOperatorInsertRandom, DictType, KeyType, NameSuffix)          \
	REGISTER_BENCHMARK(BM_SubscriptOperatorFind, DictType, KeyType, NameSuffix)  \
	REGISTER_BENCHMARK(                                                          \
			BM_SubscriptOperatorFindRandom, DictType, KeyType, NameSuffix)           \
...
REGISTER_BENCHMARK_SUITE(
		UnorderedMapLargeKeyInt, LargeKey, "UnorderedMap_Large")
