...
namespace mlir::rlc
{
			...
			llvm::raw_ostream& OS, mlir::Type t, bool ctypesSyntax = false)
	{
		...
		if (auto maybeType = t.dyn_cast<mlir::rlc::python::CTypesCharPType>())
		{
			OS << "c_char_p";
			return;
		}

		if (auto maybeType = t.dyn_cast<mlir::rlc::python::StrType>())
		{
			OS << "builtins.str";
			return;
		}
		...
		{
			OS << "POINTER(";
			writeTypeName(OS, maybeType.getUnderlying(), true);
			OS << ")";
			return;
		}
