...
# RUN: python %pyscript/learn.py %s --stdlib %stdlib --rlc rlc --total-steps 10000 -o %t --model-save-frequency 1
...
act play() -> Game:
  ...
  while are_we_done_generating != 0:
    actions:
      act connect(BInt<0, 20> source, BInt<0, 20> target) { source.value < target.value }
  ...
  while !graph.has_won() and num_actions != 100:
    ...
    actions:
      act pebble(BInt<0, 20> node) { graph.is_pebblable(node.value) }
        graph.pebble_reserve = graph.pebble_reserve - 1
        graph.nodes[node.value] = true

      act unpebble(BInt<0, 20> node) { graph.is_unpeppable(node.value) }
        graph.pebble_reserve = graph.pebble_reserve + 1
