...
class Layout:
    ...
    def child_size(self, logger: Optional['LayoutLogger'] = None, backend: Optional[ViewBackend] = None) -> None:
    ...
    def _compute_axis(self, axis: int, available: Tuple[Optional[float], Optional[float]], logger: Optional['LayoutLogger']=None, backend: Optional[ViewBackend] = None) -> None:
    ...
    def _compute_fit(self, axis: int, logger: Optional['LayoutLogger']=None, backend: Optional[ViewBackend] = None) -> float:
        self.child_size(logger, backend)
        ...
        if is_primary_axis:
            for child in self.children:
                if child.sizing[axis].size_policy == SizePolicies.GROW:
                    self._set_child_axis_size(child, axis, 0)
    ...
    def _layout_children(self, axis: int) -> None:
        ...
        if self.align == "center":
            content = sum(self._child_axis_size(c, axis) for c in self.children)
            content += max(0, len(self.children) - 1) * self.child_gap
            offset += max(0, (self._inner_size(axis) - content) / 2)
        for child in self.children:
            cross_off = self._cross_offset(child, cross)
            child_x = self.x + (offset if axis == 0 else cross_off)
            child_y = self.y + (cross_off if axis == 0 else offset)
            child.layout(child_x, child_y)
            offset += self._child_axis_size(child, axis) + self.child_gap

    def _cross_offset(self, child, cross: int) -> float:
        base = self.padding.left if cross == 0 else self.padding.top
        if self.align != "center":
            return base
        slack = self._inner_size(cross) - self._child_axis_size(child, cross)
        return base + max(0, slack / 2)
    ...
    def print_path_interaction(self, depth : int = 0):
        indent = "  " * depth
        print(f"{indent}{self.__class__.__name__}: path : {self.render_path}, on_click : {self.on_click}")
        # for k,v in self.children_mapping.items():
        #     print(f"{indent}{self.__class__.__name__}: key : {k}, value : {v.render_path}")
        for child in self.children:
            child.print_path_interaction(depth + 1)
