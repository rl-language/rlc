function(rlc TARGET_NAME)
    ...
    set(oneValueArgs DESTINATION INPUT OUTPUT HEADER PYTHON GODOT GRAPH)
    ...
    if (NOT ${RLC_GRAPH} STREQUAL "")
        add_custom_command(
            OUTPUT ${RLC_GRAPH} ${RLC_GRAPH}.dep
            COMMAND rlc ${RLC_INPUT} -o ${RLC_GRAPH} --graph ${RLC_EXTRA_ARGS} -MD
            DEPFILE ${RLC_GRAPH}.dep
            DEPENDS ${RLC_INPUT}
        )

    add_custom_target(${TARGET_NAME}_GRAPH DEPENDS ${RLC_GRAPH})
    list(APPEND CREATED_FILES ${RLC_GRAPH})
    endif()
